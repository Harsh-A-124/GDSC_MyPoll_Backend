...
def read_user(firebase_user_id: str):
    ...
    ruser_doc = db.usertest.find_one({"firebase_uid" : firebase_user_id})
    # if ruser_doc:
...
def update_user(firebase_user_id: str, update_user_data: dict):
    ...
    if user_check:
        ...
        uuser_doc = db.usertest.update_one({"firebase_uid": firebase_user_id}, {"$set": update_user_data})
        updated_user = db.usertest.find_one({"firebase_uid" : firebase_user_id})
...
def delete_user(firebase_user_id: str):
    ...
    if user_check:
        ...
        duser_doc = db.usertest.delete_one({"firebase_uid" : firebase_user_id})
...
def create_complaint(complaint: Complaints):
    ...
    if ccomplaint_dict["cid"] != "uninitialized":
        ...
        if cid_validity:
            complaint_check = db.complainttest.find_one({"_id" : ObjectId(ccomplaint_dict["cid"])})
            if not complaint_check:
                ccomplaint_doc = db.complainttest.insert_one(ccomplaint_dict)
                ccomplaint_dict["cid"] = str(ccomplaint_doc.inserted_id)
                cucomplaint_doc = db.complainttest.update_one({"_id": ccomplaint_doc.inserted_id}, {"$set": {"cid":str(ccomplaint_doc.inserted_id)}})
...
def read_complaint(complaint_id: str):
    rcomplaint_doc = db.complainttest.find_one({"cid" : complaint_id})
    if rcomplaint_doc:
...
def update_complaint(complaint_id: str, update_complaint_data: dict):
    complaint_check = db.complainttest.find_one({"cid" : complaint_id})
    if complaint_check:
        ...
        ucomplaint_doc = db.complainttest.update_one({"cid": complaint_id}, {"$set": update_complaint_data})
        updated_complaint = db.complainttest.find_one({"cid" : complaint_id})
...
def delete_complaint(complaint_id: str):
    ...
    if complaint_check:
        ...
        dcomplaint_doc = db.complainttest.delete_one({"cid" : complaint_id})
...
def create_notification(notification: Notifications):
    ...
    if cnotification_dict["nid"] != "uninitialized":
        ...
        if nid_validity:
            notification_check = db.notificationtest.find_one({"_id" : ObjectId(cnotification_dict["nid"])})
            if not notification_check:
                cnotification_doc = db.notificationtest.insert_one(cnotification_dict)
                cnotification_dict["nid"] = str(cnotification_doc.inserted_id)
                cunotification_doc = db.notificationtest.update_one({"_id": cnotification_doc.inserted_id}, {"$set": {"nid":str(cnotification_doc.inserted_id)}})
...
def read_notification(notification_id: str):
    rnotification_doc = db.notificationtest.find_one({"nid" : notification_id})
...
def update_notification(notification_id: str, update_notification_data: dict):
    notification_check = db.notificationtest.find_one({"nid" : notification_id})
    if notification_check:
        ...
        unotification_doc = db.notificationtest.update_one({"nid": notification_id}, {"$set": update_notification_data})
        updated_notification = db.notificationtest.find_one({"nid" : notification_id})
...
def delete_notification(notification_id: str):
    ...
    if notification_check:
        ...
        dnotification_doc = db.notificationtest.delete_one({"nid" : notification_id})
